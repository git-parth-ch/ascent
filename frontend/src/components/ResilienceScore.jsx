...
const ResilienceScore = ({ score, confidence, summary }) => {
  ...
  // Color logic based on animating score
  const getScoreColor = () => {
    if (displayScore >= 80) return 'text-emerald-400 stroke-emerald-500';
    if (displayScore >= 50) return 'text-amber-400 stroke-amber-500';
    return 'text-red-400 stroke-red-500';
  };

  const getScoreBg = () => {
    if (displayScore >= 80) return 'bg-emerald-950/20 border-emerald-900/30';
    if (displayScore >= 50) return 'bg-amber-950/20 border-amber-900/30';
    return 'bg-red-950/20 border-red-900/30';
  };

  const getScoreIcon = () => {
    if (displayScore >= 80) return <ShieldCheck className="w-10 h-10 text-emerald-400 animate-pulse" />;
