...
import { Zap, Clock, SkullIcon, Package, RefreshCw, AlertTriangle, ChevronDown, ChevronRight } from 'lucide-react';

const getFailureIcon = (type = '') => {
  const t = type.toLowerCase();
  if (t.includes('latency'))    return <Zap className="w-3 h-3" />;
  if (t.includes('timeout'))    return <Clock className="w-3 h-3" />;
  if (t.includes('unavailable') || t.includes('circuit_breaker') || t.includes('fail'))
    return <SkullIcon className="w-3 h-3" />;
  if (t.includes('degraded') || t.includes('dependency'))
    return <Package className="w-3 h-3" />;
  if (t.includes('retry'))      return <RefreshCw className="w-3 h-3" />;
...
const TreeNode = ({ node, isRoot }) => {
  ...
  if (isRoot) colorClass = 'text-red-600 border-red-200 bg-red-50 dark:bg-red-950 dark:border-red-900';
  else if (isLeaf) colorClass = 'text-red-700 border-red-200 bg-red-50 dark:bg-red-950 dark:border-red-900';

  return (
    <div className="flex flex-col items-start gap-1.5">
      <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded border text-[10px] font-mono ${colorClass}`}>
        <span className="select-none">{getFailureIcon(node.failure_type)}</span>
  ...
  return (
    <div className="space-y-2 select-none">
      ...
      >
        ...
        Cascade Tree ({activeEvents.length} nodes)
      </button>
      {isExpanded && (
